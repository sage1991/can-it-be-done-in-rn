...
export const HelloStickerDimensions = rect(0, 0, 300, (300 * bounds.height) / bounds.width)

interface Props {
  matrix: SkiaValue<SkMatrix>
}

export const HelloSticker: FC<Props> = ({ matrix }) => {
  return (
    <Group matrix={matrix}>
      <Group transform={fitbox("contain", bounds, HelloStickerDimensions)}>
        ...
        <Path
          ...
          strokeJoin="round"
        />
        <Path path={path} style="stroke" strokeWidth={32} strokeCap="round" strokeJoin="round" />
      </Group>
    </Group>
  )
}
